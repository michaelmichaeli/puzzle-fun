...
export class ClarifaiAPI {
  ...
  private async makeRequest<T>(endpoint: string, method: string, body?: object): Promise<T> {
  ...
  async predictImage(imageData: string, modelId: string, modelVersionId?: string): Promise<ClarifaiResponse<ClarifaiPrediction>> {
...
export const createClarifaiAPI = (config: ClarifaiConfig) => new ClarifaiAPI(config);
